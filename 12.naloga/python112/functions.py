...
def adjust_poles_and_recompute_a(s, p, type):
    ...
    if type=='Reflect':
        ...
        poles_adjusted = np.array([z if np.abs(z) <= 1 else 1 / np.conj(z) for z in poles])

    if type=='Project':
        # Step 2: Project poles outside the unit circle
        poles_adjusted = np.array([z if np.abs(z) <= 1 else 1 * z / np.abs(z) for z in poles])
...
def predict(s, p, num, type):
    ...
    R = autocorrelation(s) # Comment for all except Wolf_number I have no idea why
    a = solve_toeplitz_system(R, p) # Comment for all except Wolf_number I have no idea why
